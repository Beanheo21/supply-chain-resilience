...
try:
    from ml_risk_model import get_model as _get_model_raw, EVENT_PROFILES, FEATURES, FEATURE_DESCRIPTIONS, LABEL_COLORS, LABELS

    @st.cache_resource(show_spinner=False)
    def get_model():
        # Wrapped in st.cache_resource so Streamlit's own resource-caching
        # (which is process-wide and lock-protected) builds the model exactly
        # once and hands every session the same trained instance, rather than
        # relying solely on the plain-Python singleton/lock inside
        # ml_risk_model.py. Belt-and-suspenders fix for the concurrent-fit
        # race condition that caused "TypeError: 'int' object is not
        # subscriptable" crashes on Streamlit Cloud.
        return _get_model_raw()
...
with tab6:
    ...
    def fetch_feed(feed):
        ...
        try:
            ...
            with resp:
                raw = resp.read()
            # Strip namespaces for easier parsing. The old approach only removed
            # the xmlns declarations themselves, leaving prefixed tags like
            # <atom:link> or <media:thumbnail> with no declared namespace — which
            # made ElementTree raise "unbound prefix" ParseErrors on feeds that
            # use those prefixes (BBC, NPR, Al Jazeera, Guardian, NYT all do).
            # Now we also strip the prefixes from element and attribute names.
            raw_str = raw.decode("utf-8", errors="replace")
            raw_str = re.sub(r' xmlns(:\w+)?="[^"]*"', '', raw_str)     # drop xmlns / xmlns:prefix decls
            raw_str = re.sub(r'<(/?)[A-Za-z0-9]+:', r'<\1', raw_str)   # <atom:link> -> <link>, </atom:link> -> </link>
            raw_str = re.sub(r'(\s)[A-Za-z0-9]+:([A-Za-z0-9_-]+)=', r'\1\2=', raw_str)  # dc:creator= -> creator=
            root = ET.fromstring(raw_str.encode("utf-8"))
            items = root.findall(".//item")
            if not items:
                # Atom feeds use <entry> instead of <item>; prefixes were
                # already stripped above so no namespace lookup is needed.
    ...
    if force_refresh or (now_ts - last_fetch > CACHE_TTL) or cache_key not in st.session_state:
        with st.spinner("Scanning live intelligence feeds..."):
            # Fetch all 9 feeds concurrently instead of one at a time.
            # Sequential fetching could take up to ~90s in the worst case
            # (9 feeds x 10s timeout each) and, since Streamlit re-runs this
            # whole script top-to-bottom on every interaction, that delay hit
            # the entire app — not just this tab — until the 5-minute cache
            # refilled. A thread pool bounds worst-case wall time to roughly
            # one feed's timeout instead of the sum of all of them.
            import concurrent.futures
            all_articles = []
            feed_errors = {}
            # Hard overall deadline for the whole batch. urlopen's own
            # timeout only bounds each individual socket read, not the total
            # time to receive a response — a server that trickles bytes
            # slowly enough (or just stalls after the headers) can keep a
            # connection "alive" past its nominal timeout and hang a thread
            # indefinitely. Without a ceiling here, that one stuck feed would
            # keep the "Scanning live intelligence feeds..." spinner (and the
            # whole page, since Streamlit blocks on this script) spinning
            # forever. as_completed(timeout=...) below caps total wait time;
            # any feed still running past the deadline is simply skipped for
            # this refresh and can succeed on the next one.
            FETCH_DEADLINE_S = 20
            ...
            # submitted thread finishes regardless of the as_completed timeout
            # below, silently defeating the whole point of the deadline. We
            # shut it down manually with wait=False instead so a stuck feed's
            # thread is abandoned (not joined) once the deadline passes.
            pool = concurrent.futures.ThreadPoolExecutor(max_workers=len(FEEDS))
            try:
                future_to_feed = {pool.submit(fetch_feed, feed): feed for feed in FEEDS}
                try:
                    for future in concurrent.futures.as_completed(future_to_feed, timeout=FETCH_DEADLINE_S):
                        feed = future_to_feed[future]
                        try:
                            arts, err = future.result()
                        except Exception as e:
                            arts, err = [], f"{type(e).__name__}: {e}"
                        all_articles.extend(arts)
                        if err:
                            feed_errors[feed["name"]] = err
                except concurrent.futures.TimeoutError:
                    # Whatever hasn't finished by the deadline gets marked as
                    # timed out instead of hanging the page indefinitely.
                    for future, feed in future_to_feed.items():
                        if not future.done():
                            feed_errors[feed["name"]] = (
                                f"TimeoutError: exceeded {FETCH_DEADLINE_S}s overall fetch deadline"
                            )
                            future.cancel()
            finally:
    ...
    # ── Per-feed status — always visible, even when some feeds succeed ────
    n_feeds_ok = len(FEEDS) - len(feed_errors)
    if feed_errors:
        with st.expander(f"📡 Feed status: {n_feeds_ok}/{len(FEEDS)} sources returned articles"):
            for feed in FEEDS:
                if feed["name"] in feed_errors:
                    st.markdown(f"🔴 **{feed['name']}** — `{feed_errors[feed['name']]}`")
                else:
                    st.markdown(f"🟢 **{feed['name']}** — OK")
            st.caption(
                "If most feeds show the same error (especially a certificate or SSL error), "
                "run 'Install Certificates.command' from your Python.app folder (Mac), or "
                "`pip install --upgrade certifi`, then click Refresh Now. If only specific "
                "outlets fail while others succeed, that outlet's feed URL may have changed or "
                "started blocking automated requests — safe to ignore for now, the rest of the "
                "sources will cover you."
            )
    ...
    if not filtered:
        st.info("No articles match your filters, or feeds are temporarily unavailable. Try refreshing. "
                 "See the 📡 Feed status panel above for per-source details.")
    else:
        for art in filtered:
    ...
    if all_articles:
        ...
        with ch2:
            ...
            st.plotly_chart(fig_src_dist, width='stretch')
